...
function renderBarChart(containerId, data, options = {}) {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<p style="color: #A7ADD9; text-align: center; padding: 40px;">No data available for this period.</p>';
        return;
    }
    ...
    data.forEach((entry, index) => {
        ...
        const barOutline = createSVGElement('rect', {
            ...
            width: outerW,
            height: outerH,
            fill: entry.isMissing ? 'rgba(255,255,255,0.10)' : 'rgba(255,255,255,0.12)',
            stroke: 'none',
            'stroke-width': 0,
        ...
        if (data.length <= 31) {
            ...
            const dateLabel = createSVGElement('text', {
                x: labelX,
                y: labelY,
                fill: entry.isMissing ? '#8E95C5' : '#A7ADD9',
                'font-size': '13',
                'text-anchor': 'middle',
    ...
    points.forEach((point, index) => {
        ...
        if (data.length <= 31) {
            ...
            const dateLabel = createSVGElement('text', {
                x: labelX,
                y: labelY,
                fill: point.isMissing ? '#8E95C5' : '#A7ADD9',
                'font-size': '13',
                'text-anchor': 'middle',
                transform: `rotate(-45, ${labelX}, ${labelY})`
            });
...
function showTooltip(event, entry, metric, containerId) {
    ...
    if (entry.isMissing) {
        content += `<div style="color: #8E95C5; font-size: 11px; margin-top: 4px;">No data</div>`;
    }
...
function renderPieChart(containerId, data, field) {
    ...
    if (items.length === 0) {
        container.innerHTML = '<p style="color: #8E95C5; text-align: center; padding: 40px;">No ' + field + ' recorded in this period.</p>';
        return;
    }
...
function renderSleepBarChart(containerId, data) {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<p style="color: #8E95C5; text-align: center; padding: 40px;">No data available for this period.</p>';
        return;
    }
    ...
    if (sleepDataProcessed.length === 0) {
        container.innerHTML = '<p style="color: #8E95C5; text-align: center; padding: 40px;">No sleep data recorded in this period.</p>';
        return;
    }
...
function showSleepTooltip(event, htmlContent) {
    ...
    tooltip.id = 'chart-tooltip';
    tooltip.style.cssText = 'position: fixed; background: linear-gradient(165deg, rgba(255,255,255,0.18), rgba(255,255,255,0.06) 58%, rgba(255,255,255,0.02)), linear-gradient(135deg, rgba(20,20,31,0.88), rgba(15,22,29,0.86)); -webkit-backdrop-filter: blur(16px) saturate(140%); backdrop-filter: blur(16px) saturate(140%); border: 1px solid rgba(255,255,255,0.2); border-radius: 16px; padding: 11px 15px; color: #A7ADD9; font-size: 13px; pointer-events: none; z-index: 10000; box-shadow: 0 14px 28px rgba(5, 12, 28, 0.42), inset 0 1px 0 rgba(255,255,255,0.28); line-height: 1.6;';
